...
public class PlanetaResourceComServiceMockTest {
    ...
    public void responseDeveSerOKQuandoAtualizarValido() {
        Response response = getResource().atualizarPlaneta(PlanetaDTOMock.getMock());
        assertEquals(Response.ok().build().getStatusInfo(), response.getStatusInfo());
    }
    ...
    static Bean<?> mockService() {
        ...
        Object metodoInsercao = Mockito.when(mock.inserirPlaneta(PlanetaDTOMock.getMock())).thenReturn(new PlanetaDTO()).getMock();
        Object findIdInvalido = Mockito.when(mock.retornarPlanetaPorId(Long.valueOf(0))).thenReturn(null).getMock();
        Object findIdValido = Mockito.when(mock.retornarPlanetaPorId(Long.valueOf(1))).thenReturn(PlanetaDTOMock.getMock()).getMock();
        Object atualizar = Mockito.when(mock.atualizarPlaneta(PlanetaDTOMock.getMock())).thenReturn(new PlanetaDTO()).getMock();
        Object atualizarInvalido = Mockito.when(mock.atualizarPlaneta(null)).thenThrow(new RuntimeException()).getMock();
        Object findPorNome = Mockito.when(mock.retornarPlanetaPorNome("Teste")).thenReturn(Arrays.asList(PlanetaDTOMock.getMock())).getMock();
        Object listarTodos = Mockito.when(mock.listarPlanetas()).thenReturn(Arrays.asList(PlanetaDTOMock.getMock())).getMock();
